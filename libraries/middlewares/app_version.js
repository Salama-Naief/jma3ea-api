...
module.exports = (req, res, next) => {
	...
	const app_gallery_url = req.custom.config.mobile.app_gallery_url;
	const app_store_url = req.custom.config.mobile.app_store_url;
	const google_play_url = req.custom.config.mobile.google_play_url;

	if (api_version && ['android', 'ios', 'harmony'].indexOf(platform) > -1 && app_gallery_url && app_store_url && google_play_url && api_version > mobile_version) {
		res.out({
