  ...
  usercollection
    ...
    .then(async (theuser) => {
      ...
      if (req.body.device_token) {
        updateDeviceToken(theuser, req.body.device_token, req);
      }
      ...
      theuser.wallet =
        theuser.wallet && parseFloat(theuser.wallet) > 0
          ? parseFloat(theuser.wallet)
          : 0;
      req.custom.cache
        .set(token, data, req.custom.config.cache.life_time.token)
        .then(() => {
          res.out({
            token: token,
            user: theuser,
...
module.exports.register = function (req, res) {
  ...
  const registered_mobile_collection =
    req.custom.db.collection("registered_mobile");
  ...
  req.custom.getValidData(req).then(({ data, error }) => {
    ...
    valid_gmap.then((v_map) => {
      ...
      registered_mobile_collection
        .findOne({
          mobile: req.body.mobile,
        })
        .then((registered_mobile) => {
          data.created = common.getDate();
          data.wallet = req.custom.settings.wallet.register_gift
            ? parseFloat(req.custom.settings.wallet.register_gift)
            : 0;
          ...
          collection
            ...
            .then((response) => {
              ...
              //     data.email,
              //     data.fullname,
              //     req.custom.local.mail.registerion_subject,
              //     mail_register_view.mail_register(data, req.custom)
              //   )
              //   .catch((e) => console.error(req.originalUrl, e));
              const point_transactions_collection =
                req.custom.db.collection("point_transactions");
              ...
              const point_history_collection =
                req.custom.db.collection("point_history");
              point_history_collection
                .insertOne({
                  member_id: ObjectID(response.insertedId.toString()),
                  old_points: 0,
                  new_points: 50,
              ...
              registered_mobile_collection
                ...
                .then((registered_mobile) => {
                  ...
                  return registered_mobile_collection
                    .insertOne({
                      mobile: req.body.mobile,
                      created: new Date(),
                    })
...
module.exports.updatepassword = async function (req, res) {
  ...
  req.custom.getValidData(req).then(({ data, error }) => {
    ...
    const user_info = getInfo(req).catch((e) =>
      console.error(req.originalUrl, e)
    );
...
module.exports.forgotpassword = async function (req, res) {
  ...
  if (!userObj) {
    return res.out(
      { message: req.custom.local.mail.forgotpassword_not_found },
      ...
      .catch((e) =>
        console.error(
          ...
          e.details && e.details.length > 0
            ? e.details[0] && e.details[0].length > 0
              ? e.details[0][0]
              : e.details[0]
            : ""
        )
          ...
          .then((countryObj) => {
            ...
            if (
                ...
                .toArray((e, prods) => {
                  ...
                  for (const p of Object.keys(cart)) {
                    const product = prods.find(
                      (i) => i._id.toString() === p.toString()
                    );
                    if (
                      !product ||
                      (product.supplier && product.supplier.is_external)
                    ) {
                      delete cart[p];
                      continue;
                    }

                    const product_qty = product.prod_n_storeArr.find(
                      (i) =>
                        i.store_id.toString() === cityObj.store_id.toString()
                    );
                    if (!product_qty) {
                      delete cart[p];
                      continue;
              ...
              set_cache({
                ...
                country_id: cityObj.country_id,
                store_id:
                  cityObj.store_id || ObjectID("5d3827c683545d0366ac4285"),
                currency: countryObj.currency,
  ...
  req.custom.getValidData(req).then(({ data, error }) => {
    ...
    getInfo(req)
      .then((user) => {
        ...
        if (
          Object.keys(points_2_wallet_values).indexOf(
            req.body.points.toString()
          ) < 0
        ) {
          return res.out(
            {
        ...
        const points_to_wallet = parseFloat(
          points_2_wallet_values[req.body.points.toString()]
        );
        ...
        collection
          ...
          .then((response) => {
            ...
            wallet_history_collection.insertOne(wallet_data).then((inserted) =>
              res.out(
                {
...
module.exports.chargeWallet = async function (req, res) {
  try {
    ...
    const new_wallet = common.getFixedPrice(
      parseFloat(user_info.wallet) + parseFloat(wallet_charge_value)
    );
...
module.exports.getTheMonthShipping = async function (req, res) {
  try {
    ...
    if (
      ...
      !req.custom.settings.orders.jm3eia_pro.allow
    ) {
      return res.out(
        { message: "Free month shipping is disabled" },
        status_message.UNEXPECTED_ERROR
      );
    }
    ...
    if (
      parseFloat(req.custom.settings.orders.jm3eia_pro.shipping_cost) !== amount
    ) {
      return res.out(
        {
          message:
            "The amount you paid is not equal to the month shipping cost",
        },
        status_message.UNEXPECTED_ERROR
      );
    }
...
module.exports.sendToWallet = async function (req, res) {
  ...
  const sender_wallet =
    user_info && user_info.wallet ? parseFloat(user_info.wallet) : 0;
  ...
  const receiver = await member_collection
...
module.exports.wallet_history = function (req, res) {
  ...
  req.custom.clean_filter["member_id"] = ObjectID(
...
module.exports.list_points = async (req, res) => {
  ...
  mainController.list(
    ...
    async (out) => {
      ...
      const member_collection = req.custom.db.collection("member");
      const members_ids = out.data
        .filter((d) => d.type === "transfered" && d.sender_id)
        .map((d) => ObjectID(d.sender_id.toString()));
      const members =
      ...
      out.data = out.data.map((doc) => {
        ...
        if (typeof doc.old_points === "string")
          doc.old_points = parseInt(doc.old_points);
        if (typeof doc.new_points === "string")
          doc.new_points = parseInt(doc.new_points);
        doc.notes = req.custom.local.points_types[doc.type];
        if (doc.type === "transfered") {
          const sender = members.find(
            (m) => m._id.toString() === doc.sender_id.toString()
          );
          if (sender) {
            doc.notes += ` ${sender.mobile}`;
            doc.sender = sender;
          }
        }
    ...
    .then((response) => {
      if (response.deletedCount > 0) {
        req.custom.cache
          ...
          .catch((e) => console.error(req.originalUrl, e));
        return res.out(
          { message: req.custom.local.account_deleted },
...
module.exports.transfer_order_points = async (req, res) => {
  try {
    ...
    const order = await order_collection.findOne({
      _id: data.order_id,
      "user_data._id": ObjectID(req.custom.authorizationObject.member_id),
    });
    ...
    await order_collection.updateOne(
      { _id: order._id },
      { $set: { points_transfer_mobile: data.mobile } }
    );

    return res.out({
      message:
        "The reward points will be sent to " +
        data.mobile +
...
function fix_user_data(req, userObj, city_id) {
  ...
  address.city_id =
    address.city_id && ObjectID.isValid(address.city_id.toString())
      ? ObjectID(address.city_id.toString())
      : !req.custom.config.auto_load_city &&
        city_id &&
        ObjectID.isValid(city_id.toString())
      ? ObjectID(city_id.toString())
      : null;
        ...
        if (row_token.member_id) {
          ...
          collection
            ...
            .then((theuser) => {
              if (theuser)
                theuser.wallet =
                  theuser && theuser.wallet && parseFloat(theuser.wallet) > 0
                    ? parseFloat(theuser.wallet)
                    : 0;
              resolve(theuser);
            })
